...
const Dashboard = () => {
  ...
  // Budget by categories data
  const data = ["Transport", "Accommodation", "Food", "Shopping", "Activity", "Other"].map(category => {
  ...
  const totalPackedItems = trip.packingList.filter(i => i.packedStatus === "Packed").length;
        ...
        <div className="header-right">
          ...
          <p className="trip-date">
            {trip.itinerary.length > 0
              ? (() => {
                  const startDate = new Date(trip.itinerary[0].date);
                  const endDate = new Date(trip.itinerary[trip.itinerary.length - 1].date);
      ...
      <div className="bottom-section">
        ...
        <div className="card packing-progress">
          <div className="header">
            ...
            <a href="#" className="see-all">See all items →</a>
          </div>
          <div className="packed-summary">
            <p>
              Packed <strong>{totalPackedItems} of {totalItems} items ready</strong>
            </p>
          </div>
          {categories.map((category) => {
            const itemsInCategory = trip.packingList.filter(i => i.category === category);
            ...
            return (
              <div key={category} className="category-progress">
                <div className="category-header">
                  <span className="category-name">{category} ({packedCount}/{itemsInCategory.length})</span>
                  <span className="category-percent">{percent}%</span>
                </div>
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${percent}%`, backgroundColor: categoryColors[category] }}></div>
                </div>
              </div>
            );
          })}
        ...
        <div className="card budget-chart">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 60 }}>
              ...
              <XAxis dataKey="category" interval={0} angle={-30} textAnchor="end" />
              <YAxis />
        ...
        <div className="card budget-overview">
          ...
          <div className="progress-bar">
            <div className="progress-fill" style={{ width: `${(totalUsedBudget / trip.budget) * 100}%` }}></div>
          </div>
          ...
          <div className="card calendar-timeline">
            ...
            <div className="calendar">
              <div className="days">{["M","T","W","T","F","S","S"].map((d,i) => <div key={i}>{d}</div>)}</div>
              <div className="dates">{[27,28,29,30,1,2,3,4].map((d,i) => <div key={i}>{d}</div>)}</div>
            </div>
